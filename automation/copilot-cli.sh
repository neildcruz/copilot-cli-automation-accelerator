...
USERPROMPTEOF
        ...
        cat > "${agent_dir}/system.prompt.md" << SYSTEMPROMPTEOF
...
get_builtin_agent_config() {
    ...
    if [[ "$agent_name" == */* ]]; then
        ...
        if [[ -f "${agent_path}/user.prompt.md" ]] || [[ -f "${agent_path}/system.prompt.md" ]]; then
            has_prompt=true
        fi
        
        if [[ "$has_config" != "true" ]] && [[ "$has_prompt" != "true" ]]; then
            echo "Error: Directory does not appear to be a valid agent (no config or prompt files found)" >&2
            echo "Expected files: copilot-cli.properties, user.prompt.md, or system.prompt.md" >&2
            return 1
        fi
        ...
        if [[ -f "${agent_path}/system.prompt.md" ]]; then
            BUILTIN_AGENT_SYSTEM_PROMPT="${agent_path}/system.prompt.md"
        fi
